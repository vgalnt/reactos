...
typedef struct _USBH_OS_DESCRIPTOR {
  UCHAR Field1;
  UCHAR Field2;
  CHAR MsComp[8];
  CHAR MsSubcomp[8];
  UCHAR Unknown[6];
} USBH_OS_DESCRIPTOR, *PUSBH_OS_DESCRIPTOR;

typedef struct _USBH_OS_FEATURE_DESCRIPTOR_STUB {
  ULONG Size;
  USHORT Field1;
  USHORT Field2;
  UCHAR Count;
  UCHAR Unknown[7];
  USBH_OS_DESCRIPTOR Descriptors[0];
} USBH_OS_FEATURE_DESCRIPTOR_STUB, *PUSBH_OS_FEATURE_DESCRIPTOR_STUB;
...
typedef struct _USBHUB_COMPLETE_PORT_WAKE_IRPS_WORKER {
  WORK_QUEUE_ITEM WorkItem;
  PUSBHUB_FDO_EXTENSION HubExtension;
  LIST_ENTRY ListIrps;
  NTSTATUS NtStatus;
} USBHUB_COMPLETE_PORT_WAKE_IRPS_WORKER, *PUSBHUB_COMPLETE_PORT_WAKE_IRPS_WORKER;
